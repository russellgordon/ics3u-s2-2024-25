---
transcludeTitleSize: h2
tags:
draft: true
draftSectionTwo: false
excludeBacklinks: true
excludeFromExplorer: true
enableToc: false
createdForSectionTwo: 2025-01-30T03:34:55.000-0400
---
## Agenda
1. Quiz: SQL Basics
	- You have up to 30 minutes to demonstrate your understanding of how to write basic queries against a database.
2. Portfolio: [[Thread 3 Self-Assessment]]
	- Be sure to complete your [[Thread 3 Self-Assessment|self assessment]] before this Saturday at 10 PM.
1. Tutorial: [[To-do List App, Pt. 3]]
	- Learn to persist (keep) data even when the app is force-quit and then opened again.
	> [!TIP]
	> 
	> Optionally, choose to work ahead and [[To-do List App, Pt. 4|complete part 4]] of the tutorial series as well.

## Things to do before our next class

- [ ] Write about what you learned and made progress on today in a portfolio entry on [Notion](https://notion.so).